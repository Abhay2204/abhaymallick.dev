...
const formatMessageText = (text: string) => {
  ...
  // Headings
  html = html.replace(/(?:^|\n)###\s+(.*?)(?=\n|$)/g, '<span class="block font-extrabold text-[13px] mt-3 mb-1 text-gray-800">$1</span>');
  html = html.replace(/(?:^|\n)##\s+(.*?)(?=\n|$)/g, '<span class="block font-extrabold text-[14px] mt-4 mb-1 text-gray-900">$1</span>');
  html = html.replace(/(?:^|\n)#\s+(.*?)(?=\n|$)/g, '<span class="block font-black text-[15px] mt-4 mb-2 text-black">$1</span>');
